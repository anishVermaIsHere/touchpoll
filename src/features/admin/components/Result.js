    ...
    <Box component="main" sx={{ flexGrow: 1 }}>
      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        ...
        {allPolls.length == 0 ? (
          ...
          allPolls
            .slice(indexOfFirstPolls, indexOfLastPolls)
            .map((poll, ind) => {
              return (
                <Grid
                  container
                ...
                >
                  <Grid item xs={12}>
                    ...
                    >
                      ...
                      >
                        <Typography variant="p">{poll.question}</Typography>
                        <Button
                          variant="outlined"
                          size="small"
                          sx={{ textTransform: "none" }}
                          color="primary"
                          onClick={() => handleResults(ind)}
                        >
                          {result[ind] ? "Hide Result" : "Show Result"}
                          <BarChartIcon />
                        </Button>
                      </Typography>
                      {result[ind] && (

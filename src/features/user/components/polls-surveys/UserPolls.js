...
import {
  fetchOnePoll,
  pollSubmit,
} from "../../../../utils/services/api/polls-api";
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center" }}>
      <Box sx={{ width: "100%", mr: 1 }}>
        <LinearProgress
          sx={{ height: 15, background: "#f2f2f2", borderRadius: "12px" }}
          color="success"
          variant="determinate"
          {...props}
        />
      ...
      <Box sx={{ minWidth: 35 }}>
        <Typography variant="p">{Math.round(props.value)}%</Typography>
      </Box>
    </Box>
  );
}
...
const UserPolls = () => {
          ...
          if (res.data.voted) {
            ...
            dispatch(
              handleSnackBar({
                snackOpen: true,
                snackType: "error",
                snackMessage: "You have already voted",
              })
            );
          } else {
            setDisable(false);
          ...
          {valid ? (
            <Item elevation={0} key={poll.uid}>
              ...
              >
                ...
                >
                  Expires on {dayjs(poll.expiry_date).format('MMM D, YYYY')}
                </Typography>
                <Card
                  align="center"
                  sx={{
                              ...
                              {resultBar ? (
                                <LinearProgressWithLabel
                                  value={
                                    opt.votes == 0
                                      ? 0
                                      : (opt.votes / pollState.totalVotes) * 100
                                  }
                                />
                              ) : (
                                ""
  ...
  return pollState.poll.map((pq, ind) =>
    pollState.loading ? pollSkeleton(pq) : pollCard(pq, ind)
  );
};
